...
const COMMANDS: readonly CommandSpec[] = [
  ...
  {
    name: "sync",
    usage: ["wl sync --push [--repo owner/name] [--dry-run]"],
    summary: "Create or update GitHub issues for slices. Stores the issue number in frontmatter.",
    details: [
      "OPTIONS",
      "  --push               Push slices to GitHub (required).",
      ...
      "",
      "AUTH",
      "  Token resolution order: GITHUB_TOKEN, GH_TOKEN, then `gh auth token`.",
      "",
...
export function mainHelp(): string {
  const lines = [
    ...
    "  GITHUB_TOKEN       Token for `wl sync` (falls back to GH_TOKEN, then `gh auth token`).",
