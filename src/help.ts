...
const COMMANDS: readonly CommandSpec[] = [
  ...
  {
    name: "sync",
    usage: ["wl sync [--repo owner/name] [--dry-run]", "wl sync --push [--repo owner/name] [--dry-run]", "wl sync --pull [--repo owner/name] [--dry-run]", "wl sync --persist [--force] [--dry-run]", "wl sync --reconcile [--repo owner/name] [--dry-run]"],
    summary: "Create, update, or pull GitHub issue state for slices.",
    details: [
      "OPTIONS",
      "  (no mode)            Pull overlays first, then safely push slices to GitHub.",
      "  --push               Push slices to GitHub.",
      "  --pull               Pull issue title/state from GitHub into ignored overlays.",
      "  --persist            Write ignored overlay issue/title/state back to tracked markdown.",
      ...
      "  --force              With --persist, overwrite local issue mismatches.",
      "",
      "AUTH",
      "  Token resolution order: WORKLOG_GITHUB_TOKEN, GH_TOKEN, GITHUB_TOKEN, then `gh auth token`.",
      "",
      "Except for --persist, sync writes ignored overlays only and does not dirty tracked markdown.",
      "Without a mode flag, sync pulls remote title/state first; if markdown then differs, it does not overwrite remote unless --push is used.",
      "A slice with no linked issue gets a new issue created and the number written to its overlay.",
      "A slice linked by frontmatter or overlay updates that issue (title, body, open/closed state).",
      "Pull updates only the overlay's title/state snapshot.",
      "Persist performs no network calls; it copies ignored overlay state into tracked markdown.",
      "Reconcile lists labeled issues and matches only the strict title prefix [sl-xxxxxx], writing matches to overlays.",
    ],
  },
  {
    name: "lint",
